...
final class RoundTripTest extends TestCase
{
	...
	public function testEveryPayloadComesBack(): void
	{
		...
		$before = array_map(
			static fn(Payload $p) => DesignProjection::keyOf($p), self::$original);
		$after  = array_map(
			static fn(Payload $p) => DesignProjection::keyOf($p), self::$returned);
	...
	public function testADroppedColumnIsCaught(): void
	{
		...
		foreach (self::$returned as $payload) {
			...
			$mutated[] = new Payload($payload->entity, $payload->ownerGuid,
				$payload->relativePath, $data, $payload->isChild);
		}
	...
	{
		$mutated = array_values(array_filter(self::$returned,
	...
	private function mutate(array $payloads, string $entity, string $column, string $value): array
	{
		...
		foreach ($payloads as $payload) {
			...
			$out[] = new Payload($payload->entity, $payload->ownerGuid,
				$payload->relativePath, $data, $payload->isChild);
		}
